...
const RoadmapPage = ({ 
  ...
  useEffect(() => {
    document.title = `${roadmapData?.title || 'Learning Roadmap'} - ${siteData?.name || 'Happy Student Mentorship'}`;
  ...
  const handleMilestoneClick = (milestone) => {
    if (milestone.completed) {
      showNotification(`✅ ${milestone.title} - Well done! ${milestone.description}`, 'success');
    } else {
      showNotification(`⏳ ${milestone.title} - ${milestone.description}. Keep working towards this milestone!`, 'info');
    }
  };
  ...
  const handlePhaseAction = (action) => {
    switch (action) {
      ...
      case 'project':
        ...
        setTimeout(() => {
          showNotification('Project workspace loaded! Your mentor has left you some starting tips.', 'success');
        }, 1500);
        break;
      case 'study-group':
        showNotification('👥 Connecting you to study group...', 'info');
        setTimeout(() => {
          showNotification('Welcome to the Growth Phase study group! 3 other students are currently online.', 'success');
        }, 1500);
        break;
      default:
  ...
  return (
    <>
      {/* Hero Section */}
      <section className="hero" style={{ padding: '6rem 0 3rem', minHeight: 'auto' }}>
        <div className="container">
          <div style={{ textAlign: 'center' }}>
            ...
            <p className="hero-subtitle" style={{ marginBottom: '2rem' }}>
              {roadmapData?.subtitle || 'Track your progress through our structured mentorship program'}
            </p>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          <div className="roadmap-phases">
            {roadmapData?.phases?.map((phase, index) => (
              <div key={phase.id} className={`roadmap-phase ${phase.status} fade-in-up`} style={{ animationDelay: `${index * 200}ms` }}>
                  ...
                  <div className="milestones-list">
                    {phase.milestones?.map((milestone) => (
                      ...
                      >
                        ...
                        <div className={`milestone-badge ${milestone.completed ? '' : 'pending'}`}>
            ...
            <div className="achievements-grid">
              {achievementsData?.map((achievement) => (
                <div 
                  ...
                  onClick={() => window.showNotification?.(`🎉 ${achievement.title}: ${achievement.description}`, 'success')}
                  style={{ cursor: 'pointer' }}
                >
              ...
              <div className="achievement-card locked">
                ...
                <div className="achievement-progress">2/3 projects completed</div>
              </div>
              ...
              <div className="achievement-card locked">
                ...
                <div className="achievement-progress">Qualification in progress</div>
              </div>
            </div>
          </div>
        </div>
